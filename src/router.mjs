 ...
 *
 * This module reads NO environment. `startRouter` takes the fully resolved
 * config record that `resolveRouterConfig` (src/config.mjs) builds from the
 * host's environment at the process boundary, `bin/kimi-router.mjs` — which is
 * where the supported variables and their defaults are documented. Every value
 * the router depends on therefore arrives as a named field of one argument.
 *
...
import { nextWeeklyResetAt, parseWeeklyResetAt } from './weekly-reset.mjs';

/**
 * Run the router until it is signalled to drain.
 *
 * Owns the listening server, the state lock, and the SIGINT/SIGTERM handlers,
 * so a process hosts exactly one.
 *
 * @param {Readonly<object>} config  a record from `resolveRouterConfig`
 */
export function startRouter(config) {
  // Named for what the body has always called them; every one is now an
  // explicit input rather than an ambient read.
  const PORT = config.port;
  const HOST = config.host;
  const PROVIDER_ADAPTER = config.providerAdapter;
  const UPSTREAM = PROVIDER_ADAPTER.baseUrl;
  const STATE_FILE = config.stateFile;
  const ACCOUNTS_FILE = config.accountsFile;
  const ACCOUNTS_META_FILE = config.accountsMetaFile;
  const LEGACY_KEYS_FILE = config.legacyKeysFile;
  const KEYCHAIN_SERVICE = config.keychainService;
  const SECRET_BACKEND = config.secretBackend;
  const LOG_FILE = config.logFile;
  const MANAGEMENT_TOKEN_FILE = config.managementTokenFile;
  const STATE_LOCK_FILE = config.stateLockFile;
  const TEST_CLOCK_FILE = config.testClockFile;
  ...
  const COOLDOWN_5H = config.cooldown5hMs;
  const COOLDOWN_WEEKLY = config.cooldownWeeklyMs;
  const COOLDOWN_MONTHLY = config.cooldownMonthlyMs;
  const COOLDOWN_TRANSIENT = config.cooldownTransientMs;
  const COOLDOWN_INVALID = config.cooldownInvalidMs;
  const EXPLORATION_INTERVAL = config.explorationIntervalMs;
  const PREFERENCE_TTL = config.preferenceTtlMs;
  const RECOVERY_PROBE_INITIAL = config.recoveryProbeInitialMs;
  const RECOVERY_PROBE_MAX = config.recoveryProbeMaxMs;
  const HEADERS_TIMEOUT_MS = config.headersTimeoutMs;
  const MAX_BODY_BYTES = config.maxBodyBytes;
  const BODY_IDLE_TIMEOUT_MS = 60_000;
  const ERROR_BODY_MAX_BYTES = config.errorBodyMaxBytes;
  const MAX_INFLIGHT_PER_KEY = config.maxInflightPerKey;
  const MAX_QUEUE_DEPTH = config.maxQueueDepth;
  const QUEUE_TIMEOUT_MS = config.queueTimeoutMs;
  const DRAIN_TIMEOUT_MS = config.drainTimeoutMs;
  const LOG_MAX_BYTES = config.logMaxBytes;
  const LOG_RETAIN = config.logRetain;
  const RETRY_AMBIGUOUS_REQUESTS = config.retryAmbiguousRequests;
  ...
  function log(message, fields = {}) {
    ...
    if (config.logToStdout) console.log(entry);
    try {
  ...
  function loadManagementToken() {
    ...
    const token = normalizeManagementToken(fs.readFileSync(MANAGEMENT_TOKEN_FILE, 'utf8')); // git-secret-allow: reads a permission-checked file, no literal
  ...
  try {
    MANAGEMENT_TOKEN = loadManagementToken(); // git-secret-allow: a function call, not a literal
  } catch (err) {
  ...
  function configuredKeySourcePath() {
    if (config.keysFile !== null && config.keysFile.trim() !== '') return config.keysFile.trim();
    if (config.apiKeys !== null && config.apiKeys.trim() !== '') return null;
  ...
  function loadKeyEntries() {
    ...
    const explicitFile = config.keysFile === null ? '' : config.keysFile.trim();
  ...
  function loadWeeklyResetEpochs() {
    const byLabel = new Map();
    if (fs.existsSync(ACCOUNTS_META_FILE) === false) {
      return byLabel;
    }
    let parsed;
    try {
      parsed = JSON.parse(fs.readFileSync(ACCOUNTS_META_FILE, 'utf8'));
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      log('accounts.meta.json unreadable; weekly clocks unused', { message });
      return byLabel;
    }
    const accounts = parsed !== null && typeof parsed === 'object' && parsed.accounts !== null && typeof parsed.accounts === 'object'
      ? parsed.accounts
      : parsed;
    if (accounts === null || typeof accounts !== 'object' || Array.isArray(accounts)) {
      return byLabel;
    }
    for (const [rawLabel, rawMeta] of Object.entries(accounts)) {
      const label = String(rawLabel || '').trim().toLowerCase();
      if (label === '' || label.startsWith('_') || label === 'version') continue;
      if (rawMeta === null || typeof rawMeta !== 'object' || Array.isArray(rawMeta)) continue;
      const rawEpoch = Object.prototype.hasOwnProperty.call(rawMeta, 'weeklyResetEpoch')
        ? rawMeta.weeklyResetEpoch
        : rawMeta.weeklyResetAt;
      try {
        const epochMs = parseWeeklyResetAt(rawEpoch);
        if (epochMs !== null) byLabel.set(label, epochMs);
      } catch (err) {
        log('weeklyResetAt rejected', {
          label,
  ...
  function applyWeeklyResetEpochs(list) {
    const epochs = loadWeeklyResetEpochs();
    for (const k of list) {
      const epoch = epochs.get(k.label.toLowerCase());
      k.weeklyResetEpochMs = epoch === undefined ? null : epoch;
    }
  }

  let keys = keyEntries.map(createKey);
  applyWeeklyResetEpochs(keys);
  let nextAttemptId = 1;
  ...
  function flushState() {
    ...
    for (const k of keys) {
      out[accountStateId(k.accountId)] = {
        ...
        lastUpstreamMessage: k.lastUpstreamMessage,
  ...
  function loadState() {
    ...
    for (const k of keys) {
      ...
      if (typeof s.lastUpstreamMessage === 'string') {
        k.lastUpstreamMessage = s.lastUpstreamMessage.slice(0, 500);
      }
  ...
  function recordFailure(k, classification, attemptId, status, latencyMs, capability) {
    ...
    k.lastFailureAt = now;
    k.lastStatus = status;
    if (typeof failure.upstreamMessage === 'string' && failure.upstreamMessage !== '') {
      k.lastUpstreamMessage = failure.upstreamMessage.slice(0, 500);
    }
  ...
  function extractStructuredError(bodyText) {
    try {
        ...
        for (const field of fields) {
          ...
          if (typeof candidate === 'string' || typeof candidate === 'number') {
            ...
            if (field === 'message' && message === '') {
              message = String(candidate).slice(0, 500);
            }
          }
        }
      };
      ...
      return { normalized: values.join(' ').toLowerCase(), structured: true, message };
    } catch {
      return {
        normalized: bodyText.slice(0, ERROR_BODY_MAX_BYTES).toLowerCase(),
        structured: false,
  ...
  async function handleProxy(req, res) {
    ...
    while (attempted.size < keys.filter((k) => !k.retiring).length) {
      ...
      if (ROTATE_STATUSES.has(status) || status >= 500) {
        ...
        const extracted = extractStructuredError(text);
        if (typeof extracted.message === 'string' && extracted.message !== '') {
          cls.upstreamMessage = extracted.message;
        }
        if (
          cls.quotaWindow !== null &&
          typeof cls.quotaWindow === 'object' &&
          (cls.quotaWindow.kind === 'billing-cycle' || cls.quotaWindow.kind === 'weekly')
        ) {
          const nextReset = nextWeeklyResetAt(k.weeklyResetEpochMs, nowMs());
          if (nextReset !== null) {
            cls.cooldownMs = Math.max(1_000, nextReset - nowMs());
            cls.probeAfterCooldown = true;
            cls.quotaWindow = {
              kind: cls.quotaWindow.kind,
              resetAt: nextReset,
              source: 'console-7d',
            };
            cls.reason = `${cls.reason} (weekly reset ${new Date(nextReset).toISOString()})`;
          }
        }
  ...
  function handleStatus(res) {
    ...
    const billingCycleExhausted = keys.length > 0 && keys.every((k) =>
      accountAvailable(k, now) === false &&
      (
        (k.quotaWindow !== null && k.quotaWindow.kind === 'billing-cycle') ||
        /billing-cycle quota exhausted/i.test(k.cooldownReason)
      )
    );
    ...
    const payload = {
      ...
      summary: {
        ...
        extraUsageHint: billingCycleExhausted
          ? 'Every membership key returned Kimi billing-cycle 403. Extra Usage on that Kimi Code account is consumed automatically and would not 403; Open Platform top-up is a different product.'
          : null,
      ...
      keys: keys.map((k, index) => ({
        ...
        lastStatus: k.lastStatus,
        lastUpstreamMessage: k.lastUpstreamMessage === '' ? null : k.lastUpstreamMessage,
        weeklyResetEpoch: k.weeklyResetEpochMs === null ? null : isoOrNull(k.weeklyResetEpochMs),
        nextWeeklyResetAt: (() => {
          const next = nextWeeklyResetAt(k.weeklyResetEpochMs, now);
          return next === null ? null : isoOrNull(next);
        })(),
        latencyEwmaMs: k.latencyEwmaMs,
  ...
  if (!LOOPBACK_HOSTS.has(HOST) && !config.allowRemoteHost) {
    console.error(
